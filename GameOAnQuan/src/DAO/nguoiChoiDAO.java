...
public class nguoiChoiDAO {
    public static ArrayList<nguoiChoi> layDanhSachNguoiChoi(){
        ...
        try {
            ...
            while(rs.next()){
                ...
                nc.setTiLeThang(rs.getFloat("TILETHANG"));
                dsNC.add(nc);
            }
            provider.close();
        } catch (Exception e) {
    ...
    public static boolean themNguoiChoi(nguoiChoi nc){
         boolean kq = false;
         String sql = String.format("INSERT INTO NGUOICHOI VALUES ('%s'),'0','0','0';",nc.getTenNguoiChoi());
         DBConnect provider = new DBConnect();
         provider.open();
    ...
    public static boolean capNhatSTiLeThang(nguoiChoi nc) {
        boolean kq = false;
        String sql = String.format("UPDATE NGUOICHOI SET TILETHANG= '%f'" 
                +"WHERE TENNGUOICHOI= '%s'", nc.getTiLeThang(),nc.getTenNguoiChoi());        
        DBConnect provider = new DBConnect();
        provider.open();
        int n = provider.excuteUpdate(sql);
        if (n == 1){
    ...
    mk = mk.replaceAll("'", "''"); 
    String sql = "INSERT INTO NGUOICHOI (TENNGUOICHOI, MATKHAU, SOTRANDACHOI, SOTRANTHANG, TILETHANG) VALUES ('" + tenNC + "', '" + mk + "', 0, 0, 0.0)";
    DBConnect provider = new DBConnect();
    provider.open();
    int n = provider.excuteUpdate(sql);
    if (n == 1) {
